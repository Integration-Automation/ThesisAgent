...
> `paper_rule`, plus the task-running agents `dod-verify`,
...
ad-hoc path) — you MUST read `.claude/agents/deck-design.md` AND
`.claude/agents/slide-deck-rules.md` first**, in the same turn as the edit.
...
| Academic paper writing rules (multilingual, all 14 locales) — Abstract / Introduction / Literature Review / Methodology / Experiment / Conclusion / References structure + PaperSummary-to-section mapping | `paper_rule` |
