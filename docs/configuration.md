...
| `AUTOPAPERTOPPT_DISABLE_WEBRUNNER` | unset | When the `[scholar]` extra is installed (`pip install autopapertoppt[scholar]`), the Scholar plugin auto-uses **WebRunner** (real Chrome via Selenium) instead of httpx — Google's bot-detection is far less aggressive on real browsers. Set `=1` to force the httpx scrape path even when WebRunner is installed (useful for CI where Chrome adds 5-10 s overhead). |
