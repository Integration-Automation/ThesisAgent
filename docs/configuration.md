...
| `AUTOPAPERTOPPT_DISABLE_WEBRUNNER` | unset | **Scholar and IEEE plugins both default to driving a real visible Chrome through WebRunner** (`je_web_runner` is a default dependency) — publisher bot-detection is far less aggressive on real browsers. Set `=1` to force the httpx scrape paths instead (useful for CI / Docker containers without a Chrome binary). When set, Scholar's captcha-cooldown and IEEE's 403 fallback paths still apply. |
