...
- **Recorded fixtures**: `tests/fixtures/<source>/<scenario>.{json,html,xml}`. Re-record by hand — save the upstream response into that directory and commit it. Strip user-specific tokens before committing.
...
| Paywalled PDFs (`ieeexplore.ieee.org`, `dl.acm.org`, `link.springer.com`, `sciencedirect.com`, `onlinelibrary.wiley.com`, `tandfonline.com`, `academic.oup.com`, `nature.com`, `science.org`) | n/a — search lives in another source | The pipeline downloader (`thesisagents/core/pdf_download.py`) routes these hosts through `fetchers/webrunner_pdf.py` (visible Chrome) automatically; for papers with no `pdf_url`, LLM-driven Bash + Selenium via `thesisagents.fetchers.webrunner_browser.make_driver()` — see `paper-summary-author.md` "When the CLI couldn't download a paywalled PDF". No `mcp__webrunner__*` server is registered for this project, and where it is registered it exposes only static helpers (lint/translate/score), not browser-driving actions, so do not assume those are available. |

**In practice:**

1. **Confirm VPN access BEFORE running any search that involves IEEE / ACM / Springer / paywalled-PDF flows.** When the user requests a paper search ("搜尋 X" / "search X" / "find papers on X"), the LLM's first action is NOT to invoke the search — it is to check VPN status. Either recall from the conversation, or ask via `AskUserQuestion` ("Do you have VPN / institutional access for IEEE / ACM / Springer for this topic? Affects whether I include `ieee` as a source and whether per-paper PDF download will work."). Without VPN: IEEE returns abstract-only / 403 for the PDF stage and the user wastes time on a Chrome window that can't reach the content. Same gate applies before the MCP `search` / `download_pdfs` tools touch `ieee` or paywalled-publisher PDFs, and before any hand-written Bash + Selenium capture script. When the user confirms NO VPN, restrict the source mix to every default source except `ieee` — skip ONLY `ieee`. Google Scholar is publicly accessible (no subscription needed) and stays in the mix even without VPN; Chrome still boots for it because of Google's captcha resilience, but the SERP itself works fine.
...
4. LLM-as-agent paywalled PDF fetch: drive a one-off Bash + Selenium capture script — `webrunner_browser.make_driver()` for visible Chrome, `driver.get(...)`, `wait_for_captcha_solved(...)`, capture `driver.page_source` or trigger a real download (worked per-publisher routes in `paper-summary-author.md`). Never paste a publisher URL into `httpx` / `urllib` / `subprocess curl` and call it equivalent. Never call `mcp__webrunner__webrunner_run_actions`: no MCP server exposing it is registered for this project.
