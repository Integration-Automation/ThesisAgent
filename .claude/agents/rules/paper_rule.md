...
3. **If the venue is ambiguous** (e.g. you see `TCSE_v2.3.docx` but don't know whether the author means IEEE TCSE or just a local thesis style), **ask via `AskUserQuestion` before rewriting any reference**. The cost of pausing is low, and the cost of converting 22 entries the wrong direction is high.
...
- Citation number is **fixed** at first mention, and subsequent mentions reuse the same number.
...
- Title in **Title Case** (this is the IEEE convention, while the `compress_tcse_v22.py` original references preserve the venue-supplied casing whether Title Case or sentence case).
...
- DOI preferred over URL when both exist, and formatted as `https://doi.org/…` (no `doi:` prefix).
...
### First-use is mandatory, not "first prominent use" / 首次出現即須解釋

The rule is **first chronological appearance in the document**, not "first use in the sub-section where the term gets a full treatment". Common failure mode: the author uses `ADA` casually on page 2 ("規則式 ADA 在多樣 benchmark 上掉到 65.4%"), then defines it properly on page 8 inside the §4.2 evaluation-metrics block. By page 8 the unprepared reader has either Googled it or stopped following — the late definition is wasted. The same applies inside a slide deck: if `ADA` first appears on slide 3 as a pain-point bullet, that's where the gloss belongs, and defining it on slide 9 is too late. **Audit by grepping the document linearly for each technical term and confirming its first hit is also the glossed one.**

Same rule for **math notation**: the first occurrence of `I(za;zb|Ep)` must carry the operator name (`互資訊 I(za;zb|Ep)`) — symbol-only notation reads as "some math" to anyone not already inside the sub-field. For **library / model / tool names** the gloss must include provenance: `FAISS(Facebook AI 釋出之向量索引庫)` rather than `FAISS` alone, since the reader can't tell otherwise whether it's a model, dataset, or service.
...
2. **Self-referential gloss** — `LoRA (Low-Rank Adaptation)` alone does not help, because the reader still doesn't know what low-rank adaptation does. Always add a one-clause **function description**. / 自指式解釋（只展開縮寫不說功能）= 沒解釋。
...
## 不謊造、不幻覺、不偽造資料 / No fabrication, no hallucination, no fake data (HARD RULE — most fundamental)

**EN.** Every empirical claim in a paper — numbers, table cells, percentages,
p-values, sample sizes, dataset sizes, hyperparameter settings, experimental
configurations, names of compared methods, citations, dates, author names —
MUST come from a verified source: the actual experiment log / dataset / code
run that the author or co-authors performed, or a published reference they
have personally read. Inventing a plausible-looking number, a plausible-
looking RQ, or a plausible-looking comparison table to "fill out" a paper
is research misconduct and is forbidden without exception.

**ZH-TW.** 論文中之任何實證宣稱（數字、表格數值、百分比、p 值、樣本數、
資料集大小、超參數、實驗配置、比較方法名稱、引文、日期、作者姓名）皆
**必須**來自可驗證之來源：作者或共同作者實際執行之實驗 log／資料集／
程式碼結果，或作者本人已讀過之已發表文獻。為「填滿」論文而捏造看似
合理之數字、看似合理之研究問題、看似合理之比較表，屬於研究不當行為，
任何情況下皆禁止。

### Specific forbidden practices / 具體禁止行為

1. **不得編造任何實驗數據**。包括但不限於 precision / recall / F1 /
   accuracy / cost / latency / p50 / p95 / 標準差 / 信賴區間 / Δ 值。
   若該數字未由作者親自跑出，不可寫入論文。
2. **不得編造研究問題（RQ）**。每個 RQ 都必須對應到實際進行之實驗段落
   並有可重現之結果。沒有實驗就沒有 RQ。
3. **不得編造比較對象**。表格內每一個 baseline、每一個 backend、每一個
   model variant 都必須是作者親自跑過或在 cited 文獻中明確記載者。不
   可加入「邏輯上應該存在」但未實際執行之比較欄位。
4. **不得編造參考文獻**。每筆引文都必須是作者親自確認存在且讀過之文獻。
   看 title 像是真的、看 venue 像是真的、看作者像是真的，不代表它存在；
   需以 DOI / arXiv ID / ISBN / 官方 venue 程式集驗證。
5. **不得編造系統參數**。閾值（如 τ = 0.85）、top-K（如 K = 3）、
   threshold（如 0.7）若未由作者在程式碼或實驗中實際使用過、且未在實驗
   結果中對該設定產出可重現之數字，不可寫入論文做為「本研究採用」之
   配置陳述。
6. **不得編造日期、版本、機構名**。timestamp、git commit、模型 release
   日期、機構合作關係等若不確定，請查證或不提，不可猜測填入。
7. **不得以「合理推估」掩飾捏造**。「以電力 0.15 USD/kWh 與 350 W 估算」、
   「30 次重複實驗之標準差皆 < 0.02」此類具體數字若未實際量測，即便看
   似合理也屬於捏造。
8. **不得編造既有論文中根本沒有之段落或結論**。引用某篇文獻時，所摘述
   之觀點必須真實出現於該文，不可推測作者「應該會這樣說」。

### Acceptable framings for "code exists but no experiment yet" / 程式碼存在但未進行實驗時之合法框架

研究過程中常見「framework 已實作某機制，但未對其單獨評估」之情境。此時：

- **可寫**：「本研究隨附之開源框架實作了 X 機制（見 §3.Y 之設計描述
  與附錄 A 之程式碼引用）；其量化效益之驗證留待未來工作（見 §6.4）」。
- **可寫**：「本框架設計上支援 Y，惟本研究之實驗範圍僅涵蓋 Z；對 Y
  之獨立評估屬未來工作」。
- **可寫**：「本框架同時支援 backend A、B、C；本研究之實驗以 A 為主，
  B 與 C 之比較留待後續評估」。
- **不可寫**：「本研究於 X 機制下取得 precision 0.83」（若該 0.83 不存在）。
- **不可寫**：「RQ：X 機制是否提升 Y？答：是，提升 +12%」（若該實驗未進行）。
- **不可寫**：「在 100 次 PR 之累積資料上，X 機制使指標由 A 升至 B」
  （若該資料未收集）。
- **不可寫**：「Backend B 之延遲為 p50 980 ms、p95 1280 ms」（若該 backend
  從未實際被 benchmark 過）。

判斷準則：若一名審稿人要求作者提供 raw data / log / code 以驗證該數字，
作者是否能立刻提供？若否，就不可寫入論文。

### Self-audit / 自我檢查

完成一段含實證宣稱之段落後依序問：

- [ ] 本段所述之每一個數字，我能否指出它來自哪份 .log / .csv / 表格？
- [ ] 本段所述之每個 RQ，是否對應到一個我親手執行過之實驗？
- [ ] 本段所引之每筆 [N]，我是否親自讀過該篇文獻且其 DOI / arXiv ID
      / venue 可被搜尋驗證？
- [ ] 本段所提之每個 baseline / 方法名稱，是否為我親自跑過或引文中明確
      記載？
- [ ] 本段所提之每個閾值 / top-K / threshold / 超參數，是否為我實際使用
      過之數值，且該設定下之結果有出現於表格中？
- [ ] 本段所提之每個 confidence interval / 標準差 / p-value，是否為我
      實際計算過之統計量？

若任一項回答為「否」，刪除該句或將其改寫為「框架設計貢獻 / 未來工作」
框架，並明確標示「本研究未對此進行量化評估」。

### Why this matters / 為何此規則最為基本

論文之根本價值在於「可被獨立驗證」。一旦某段內容無法被驗證，整篇論文
之可信度即受質疑——即便其他段落皆為真實。捏造之代價遠高於誠實標示
「未驗證」之代價。在期刊投稿環境中，被審稿人或讀者發現捏造數據等同
於 retraction risk；在學位論文環境中，可被認定為學術不誠實。

**任何 AI 協作工具（含本 agent set）若為「填滿論文」而產出看似合理但
無證據之內容，皆屬於違反此規則。作者收到 AI 產出之段落後，必須對每
個實證宣稱執行上述 self-audit；無法通過者一律移除或改寫為未來工作。**

> 本規則優先序最高。當其與「不重複」、「無 AI 口頭禪」、「圖表必須解釋」
> 等其他硬規則衝突時，本規則優先 —— 寧可省略某段、寧可標示「未驗證」、
> 寧可改為未來工作，也不可為了形式完整而填入捏造內容。
