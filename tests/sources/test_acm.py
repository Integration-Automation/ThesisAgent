...
from autopapertoppt.sources.acm.fetcher import AcmFetcher
...
async def test_search_filters_to_acm(monkeypatch):
    ...
    install_mock(monkeypatch, "autopapertoppt.sources.acm.fetcher", transport)
...
async def test_crossref_plus_token_attached_when_env_set(monkeypatch):
    ...
    install_mock(monkeypatch, "autopapertoppt.sources.acm.fetcher", _CaptureTransport())
    await AcmFetcher().search(
        Query(keywords="x", sources=("acm",), max_results=1)
    )
