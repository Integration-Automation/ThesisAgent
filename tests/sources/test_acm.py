...
from thesisagents.sources.acm.fetcher import AcmFetcher
...
async def test_search_filters_to_acm(monkeypatch):
    ...
    install_mock(monkeypatch, "thesisagents.sources.acm.fetcher", transport)
...
async def test_mailto_sent_when_env_set(monkeypatch):
    monkeypatch.setenv("THESISAGENTS_CONTACT_EMAIL", "polite@example.org")
    transport = MockTransport(200, '{"message": {"items": []}}')
    install_mock(monkeypatch, "thesisagents.sources.acm.fetcher", transport)
    await AcmFetcher().search(
...
async def test_crossref_plus_token_attached_when_env_set(monkeypatch):
    """``THESISAGENTS_CROSSREF_PLUS_TOKEN`` must turn into a
    ``Crossref-Plus-API-Token: Bearer …`` header on every request."""
    import httpx

    monkeypatch.setenv("THESISAGENTS_CROSSREF_PLUS_TOKEN", "plus-secret")
    ...
    install_mock(monkeypatch, "thesisagents.sources.acm.fetcher", _CaptureTransport())
    await AcmFetcher().search(
        Query(keywords="x", sources=("acm",), max_results=1)
    )
...
async def test_crossref_plus_token_absent_when_env_unset(monkeypatch):
    ...
    monkeypatch.delenv("THESISAGENTS_CROSSREF_PLUS_TOKEN", raising=False)
    captured: dict[str, str] = {}
