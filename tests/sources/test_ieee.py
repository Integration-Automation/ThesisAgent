...
def _new_fetcher():
    from autopapertoppt.sources.ieee.fetcher import IeeeFetcher
...
async def test_search_parses_rest_response(monkeypatch):
    ...
    install_mock(monkeypatch, "autopapertoppt.sources.ieee.fetcher", transport)
...
async def test_search_sends_referer_and_origin(monkeypatch):
    ...
    install_mock(monkeypatch, "autopapertoppt.sources.ieee.fetcher", _CaptureTransport())
    await _new_fetcher().search(
        Query(keywords="x", sources=("ieee",), max_results=1)
    )
...
async def test_webrunner_search_used_when_available(monkeypatch):
    ...
    from autopapertoppt.sources.ieee import webrunner_backend
