    ...
    for style_name, text in items:
        if style_name in ("Heading 2", "Heading 3"):  # NOSONAR OOXML/Word style literal; a constant adds no value in this one-shot script
            ...
            cursor = _new_paragraph_after(cursor, text,
                                          style_name=style_name or "Normal (Web)",  # NOSONAR OOXML/Word style literal; a constant adds no value in this one-shot script
...
def update_thesis() -> None:  # NOSONAR one-shot anchor-driven rewrite script; branchy by design (cf. ruff C901 scripts exemption)
    d = Document(THESIS)
    before = sum(len(p.text) for p in d.paragraphs)
