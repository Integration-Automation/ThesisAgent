"""Canonical slide-overflow inspector for ThesisAgents decks.

Why this script exists
----------------------
The `slide-overflow-check` subagent is supposed to "reuse the project's existing
inspector" — but no such file existed, so the agent had to reinvent an estimator
from a stub every run. This is that inspector: one runnable, importable module the
agent (and a regression test) can call.
...
a *rough* estimate — deliberately conservative, the same trade-off the manual
agent made — so it catches gross overflow without needing a font-metrics library.
