...
- **Восемь экспортёров**:
  ...
  - **Визуальная идентичность оформленного дека** (не стандартный вид Calibri-на-
    ...
    UI / YaHei UI / Yu Gothic UI / Malgun Gothic / Nirmala UI для
    CJK + хинди), программная акцентная геометрия (верхняя акцентная полоса на каждом
...
- **`release.yml`** ждёт завершения `ci.yml` на `main`
  ...
  4. **`build-nuitka`** — скомпилировать автономный бандл Nuitka на
     ...
     сокращает тёплые сборки с ~85 мин холодной до ~5–10 мин.
...
4. Выпускайте релизы, вливая PR в `main`. Конвейеру нужно
   ~3–5 мин, чтобы опубликовать на PyPI, и ещё ~80–90 мин (холодно) или ~5–10 мин
