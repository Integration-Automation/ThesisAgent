...
Trợ lý tìm kiếm bài báo theo từ khóa. Lấy kết quả từ arXiv, Semantic Scholar, OpenAlex, PubMed, ACM (qua Crossref), IEEE Xplore, DBLP, Crossref tổng quát, OpenAIRE, Springer Nature, Europe PMC, DOAJ, HAL, CORE và Google Scholar; chuẩn hóa về một định dạng bản ghi duy nhất; và xuất tập đã khử trùng lặp thành **slide PowerPoint phong cách luận văn**, **sổ Excel** và **tệp BibTeX** — tất cả từ một lệnh CLI hoặc một lời gọi công cụ MCP. Có thể làm giàu mỗi bài báo bằng cách đọc PDF và tạo bản tóm tắt có cấu trúc, ngay trong ngữ cảnh (luồng LLM-as-agent) hoặc qua API Anthropic (luồng Python pipeline).
...
| `--export` / `-e` | Định dạng: tổ hợp bất kỳ của `pptx,xlsx,md,bib,json,ris,csv,csl`. Mặc định tùy chế độ (xem dưới). |
...
| `THESISAGENTS_S2_API_KEY` | Semantic Scholar | Rate limit cao hơn. Tùy chọn. |
