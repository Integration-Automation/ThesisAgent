...
Default-on; opt out with ``THESISAGENTS_DISABLE_SCHOLAR_SCRAPING=1``.
...
_OPT_OUT_ENV = "THESISAGENTS_DISABLE_SCHOLAR_SCRAPING"
...
class ScholarFetcher(Fetcher):
    ...
    def __init__(self) -> None:
        super().__init__()
        # Scholar is default-on; flip off via THESISAGENTS_DISABLE_SCHOLAR_SCRAPING=1.
    ...
    async def _get_text(self, url: str, *, params: dict[str, str]) -> str:
        ...
        if _is_captcha_response(str(response.url), response.text):
            ...
            raise SourceUnavailableError(
                ...
                "THESISAGENTS_DISABLE_SCHOLAR_SCRAPING=1 to skip the "
