...
2. **Unpaywall** (https://api.unpaywall.org/v2). Free, no API key,
   ~50M papers. Needs ``THESISAGENTS_CONTACT_EMAIL`` for politeness
...
from thesisagents.core.exceptions import FetchError
...
async def _query_unpaywall(doi: str) -> str | None:
    ...
    email = os.environ.get("THESISAGENTS_CONTACT_EMAIL", "").strip()
    if not email:
...
async def _query_semantic_scholar(doi: str) -> str | None:
    ...
    Honours ``THESISAGENTS_S2_API_KEY`` (sent as the ``x-api-key``
    ...
    api_key = os.environ.get("THESISAGENTS_S2_API_KEY", "").strip()
    if api_key:
...
async def _query_core(doi: str) -> str | None:
    ...
    api_key = os.environ.get("THESISAGENTS_CORE_API_KEY", "").strip()
    if not api_key:
...
def _warn_once_about_email() -> None:
    ...
    _LOG.warning(
        "OA resolver: THESISAGENTS_CONTACT_EMAIL is not set; "
...
def _warn_once_about_core() -> None:
    ...
    _LOG.info(
        "OA resolver: THESISAGENTS_CORE_API_KEY is not set; CORE.ac.uk "
