"""MCP tool surface for ThesisAgents.
...
- list_exports() -> {formats: [...]}
  Discovery tool symmetric to ``list_sources``: report every export format
...
_PLUGIN_OPT_IN_ENV: dict[str, tuple[str, ...]] = {
    "springer": ("THESISAGENTS_SPRINGER_API_KEY",),
    "core": ("THESISAGENTS_CORE_API_KEY",),
}
...
def _register_discovery_tools(server: FastMCP) -> None:
    ...
    def list_sources() -> dict[str, Any]:
        ...
        - **ieee** — default-ON. Set ``THESISAGENTS_DISABLE_IEEE_SCRAPING=1``
          to opt out, or ``THESISAGENTS_IEEE_API_KEY`` for the official
          Xplore API (better metadata + pdf_url for subscribers).
        - **scholar** — default-ON. Set ``THESISAGENTS_DISABLE_SCHOLAR_SCRAPING=1``
          to opt out (Google's ToS forbids automated access; default-on
          for coverage, accept the risk).
        - **springer** — opt-IN via ``THESISAGENTS_SPRINGER_API_KEY``.
    ...
    def list_exports() -> dict[str, Any]:
        ...
        Symmetric to ``list_sources``: call it once before ``export`` so you
        pass only recognised formats. Each entry has ``format`` (the string you
        put in ``export``'s ``formats`` list), a human ``description`` and an
        ``aggregate`` flag — ``aggregate=true`` writes one file for the whole
        run (xlsx / md / bib / json / ris / csv), while ``pptx`` and ``pdf`` are
        emitted per paper.
        """
        formats = [
            {
                "format": fmt,
                "description": EXPORT_DESCRIPTIONS[fmt],
                "aggregate": fmt in AGGREGATE_EXPORTS,
            }
...
def _register_pdf_download_tool(server: FastMCP) -> None:
    ...
    async def download_pdfs(
        ...
        Wraps ``thesisagents.core.pdf_download.download_pdfs``. Skips
        ...
        """
        if not papers:
            raise ThesisAgentsError("download_pdfs requires at least one paper")
...
def _register_search_tools(server: FastMCP) -> None:
    ...
    async def search(
        ...
        ``exclude_sources`` is subtracted AFTER ``sources`` resolves, so the
        common no-VPN gesture is to leave ``sources`` at its default and pass
        ``exclude_sources=["ieee"]`` — every other default source stays in.
        Excluding everything is an error.
        ...
        if exclude_sources:
            ...
            chosen = tuple(s for s in chosen if s not in excluded)
            if not chosen:
                raise ThesisAgentsError(
                    "exclude_sources removed every source — nothing left to search"
                )
        query = Query(
            keywords=normalised,
            sources=chosen,
    ...
    async def fetch_paper(identifier: str) -> dict[str, Any]:
        ...
        if not collection.papers:
            raise ThesisAgentsError(f"no paper found for identifier {identifier!r}")
        return {
