"""argparse-based CLI for ThesisAgents.

Usage:
    py -m thesisagents --query "diffusion models" --source arxiv \\
...
from thesisagents.core.pipeline import (
    enrich_collection,
    run_search,
    run_single_paper,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Launching the desktop GUI: run `thesisagents` with no arguments, "
            "or `thesisagents gui`, or the standalone `thesisagents-gui` entry "
            "point. The GUI extras must be installed: pip install "
            "thesisagents[gui]."
        ),
    )
    parser.add_argument(
        "--version", action="version", version=f"thesisagents {__version__}"
    )
    # Discovery flags. Declared here for --help visibility, but actually handled
    # in main() BEFORE parse_args so they don't trip the required mode mutex.
    parser.add_argument(
        "--list-sources",
        action="store_true",
        help="Print the available search sources and exit.",
    )
    parser.add_argument(
        "--list-exports",
        action="store_true",
        help="Print the available export formats and exit.",
    )
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Comma-separated sources to remove from the mix (used with --query). "
            "Subtracted AFTER --source resolves, so the common case is leaving "
            "--source at its default and pruning a few: e.g. without VPN, "
            "`--exclude-source ieee` keeps every other default source. Names "
            "must be valid (see --source); an unknown name is an error."
        ),
    )
    parser.add_argument(
        "--max",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: claude-opus-4-7 (or THESISAGENTS_LLM_MODEL)."
        ),
    )
    parser.add_argument(
        "--light-mode",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "in Unpaywall (needs THESISAGENTS_CONTACT_EMAIL) and falls "
...
def _resolve_sources(args: argparse.Namespace) -> tuple[str, ...]:
    """Resolve the final source mix: ``--source`` minus ``--exclude-source``.

    Both lists are validated against ``ALL_SOURCES`` first, so a typo in either
    flag fails loudly instead of silently doing nothing (an unknown name in
    ``--exclude-source`` would otherwise be a no-op the user never notices).
    Excluding every source is rejected — an empty mix can't search.

    Example: ``--exclude-source ieee`` on the default mix is the documented
    no-VPN path — it keeps arxiv/openalex/pubmed/crossref/… and drops only the
    paywalled publisher that needs institutional access.
    """
    sources = _parse_csv(args.source)
    _validate_sources(sources)
    excludes = _parse_csv(getattr(args, "exclude_source", "") or "")
    _validate_sources(excludes)
    if not excludes:
        return sources
    remaining = tuple(s for s in sources if s not in excludes)
    if not remaining:
        raise SystemExit(
            "All sources were excluded — nothing left to search. "
            f"--source={','.join(sources)} minus "
            f"--exclude-source={','.join(excludes)} is empty."
        )
    return remaining
...
def _dispatch_gui(gui_argv: list[str]) -> int:
    ...
    try:
        from thesisagents.gui.app import main as gui_main
    except ImportError as err:
        print(
...
def _print_sources() -> None:
    """Print the source catalog for ``--list-sources``.

    Kept dependency-free (constants only, no MCP import) so the flag works in a
    bare ``pip install thesisagents`` without the optional ``mcp`` extra. The
    ``[default]`` marker tells the user which sources run when ``--source`` is
    omitted — the rest must be named explicitly.
    """
    print("Available sources (use with --source / --exclude-source):")
    for name in ALL_SOURCES:
        marker = " [default]" if name in DEFAULT_SOURCES else ""
        print(f"  {name}{marker}")
    print(f"\nDefault mix: {','.join(DEFAULT_SOURCES)}")


def _print_exports() -> None:
    """Print the export-format catalog for ``--list-exports``."""
    print("Available export formats (use with --export):")
    for fmt in ALL_EXPORTS:
        kind = "aggregate" if fmt in AGGREGATE_EXPORTS else "per-paper"
        print(f"  {fmt:<5} ({kind}) — {EXPORT_DESCRIPTIONS[fmt]}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    # without this shim, ``thesisagents`` (no args, the obvious
    ...
    if "--list-sources" in raw_argv:
        _print_sources()
        return 0
    if "--list-exports" in raw_argv:
        _print_exports()
        return 0
    parser = build_parser()
